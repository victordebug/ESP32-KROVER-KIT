...
#define BLE_RECV_DATA(N)   ( *((uint8_t*)param->write.value + N) )
uint8_t* BLE_Recv_Buffer =NULL;
...
uint8_t ble_recv_cut = 0;
int8_t  ble_needrecv_len = 0;
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
		...
		uint8_t check_data = 0;
		 ...
		 if(ble_needrecv_len > 0)
		 {
			    ble_needrecv_len -= 20;
				ESP_LOGD("BLE", "ble_needrecv_len:%d",ble_needrecv_len); 
			    for(uint8_t i=0;i<param->write.len; i++)
				   BLE_Recv_Buffer[ble_recv_cut++] = *((uint8_t*) param->write.value + i);	    							 
		 }
		else
		{
			//数据从APP发送接收 
			if(*((uint8_t*) param->write.value + 3) == 0x02){
				
				Frame_data_len = (uint16_t)(BLE_RECV_DATA(5))<<8 | (uint16_t)(BLE_RECV_DATA(6));
				ESP_LOGD("BLE", "Frame_data_len%d",Frame_data_len); 

				if(Frame_data_len != 0 ){
					if(Frame_data_len > 100 ){
						ESP_LOGD("BLE", "Recv data err");
						return;
					}
					...
					if(Frame_data_len > 12 ){
						ble_needrecv_len = Frame_data_len - 12; 
						ESP_LOGD("BLE", "ble_needrecv_len:%d",ble_needrecv_len); 
					}	
					else{
						ble_needrecv_len = 0;	
...
//  以上接收在将多个ble数据包组成完整一个帧

 
		if(ble_needrecv_len <= 0 && Frame_data_len != 0){
			ble_recv_cut = 0;
			for(uint8_t i = 0; i< Frame_data_len + 8; i++){			
				ESP_LOGD("BLE", "recv data:%x",BLE_Recv_Buffer[i]);
			}

			for(uint8_t i=0;i<Frame_data_len;i++)
				check_data=check_data + BLE_Recv_Buffer[7+i];
			ESP_LOGD("BLE", "check_data%d",check_data);
			
			if(check_data == (BLE_Recv_Buffer[7 + Frame_data_len]))
			{
				ESP_LOGD("BLE","Data OK");

				if(BLE_Recv_Buffer[7] == 'E'){
					uint8_t* ble_data_p = Ble_get_data(BLE_Recv_Buffer,Frame_data_len);

					Ble_data_analyexe(ble_data_p,Frame_data_len,param); 
					Frame_data_len = 0;
				}
				else{
					UARTframe  frame = {
					.framedata_type = Type_BLE,
					.framedata_len = Frame_data_len,   //Data_len
					.framedata_p = (uint8_t*)BLE_Recv_Buffer + 7 ,	 //data_p
					.framedata_reply = 0,
					};
					if(BLE_RECV_DATA(2) & 0x01){
			...
			{
				ESP_LOGD("BLE","Data err");
				uint8_t Send_data[] = {"U_err"};
				uint8_t* frame_pack = ble_frame (0,0,0,1,0, strlen((char*)Send_data), Send_data );
				ALble_send (gatts_if, 8+strlen((char*)Send_data), frame_pack );
				Frame_data_len = 0;
			}		
			...
			if(*(uint8_t*)param->write.value == 0x01){
//				uint8_t device_id[] = {Device_id};
//				uint8_t* frame_pack = ble_frame (0,0,0,1,0, 13, device_id );
//				ALble_send (gatts_if, 21, frame_pack );
    ...
    case ESP_GATTS_CONNECT_EVT: {
        ...
        conn_params.timeout = 300;    // timeout = 300*10ms = 3000ms
